...
func (f *Filter) Filter() (*[]RecipeSchema, *error_handler.APIError) {
	...
	if f.Ingredients != nil {
		query = query.Where("ingredients_schemas.ingredient IN ?", f.Ingredients).
	...
	case f.CookingTime > 0:
		query = query.Where("recipe_schemas.cooking_time <= ?", f.CookingTime)
	case f.NutriScore != "":
		query = query.Where("recipe_schemas.nutri_score = ?", f.NutriScore)
	}
